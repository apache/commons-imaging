 ...
 * 
 * For the most recent information on format support for the Apache Commons
 * Imaging package, refer to <a
 * href="http://commons.apache.org/imaging/formatsupport.html">Format
 * Support</a> at the main project development web site.
 * 
 * <h4>On the accuracy of this Javadoc</h4>
 * 
 * The original authors of this class did not supply documentation. The Javadoc
 * for this class is based on inspection of the source code. In some cases, the
 * purpose and usage for particular methods was deduced from the source and may
 * not perfectly reflect the intentions of the original. Therefore, you should
 * not assume that the documentation is perfect, especially in the more obscure
 * and specialized areas of implementation.
 * 
 * <h4>The "Map params" argument</h4>
 * 
 * Many of the methods specified by this class accept an argument of type Map
 * giving a list of parameters to be used when processing an image. For example,
 * some of the output formats permit the specification of different kinds of
 * image compression or color models. Some of the reading methods permit the
 * calling application to require strict format compliance. In many cases,
 * however, an application will not require the use of this argument. While some
 * of the ImageParser implementations check for (and ignore) null arguments for
 * this parameter, not all of them do (at least not at the time these notes were
 * written). Therefore, a prudent programmer will always supply an valid, though
 * empty instance of a Map implementation when calling such methods. Generally,
 * the java HashMap class is useful for this purpose.
 * 
 * <p>
 * Additionally, developers creating or enhancing classes derived from
 * ImageParser are encouraged to include such checks in their code.
 */
public abstract class ImageParser extends BinaryFileParser {
	...
	public static ImageParser[] getAllImageParsers() {
		...
		return new ImageParser[] { new BmpImageParser(), new DcxImageParser(),
				new GifImageParser(), new IcnsImageParser(),
				new IcoImageParser(), new JpegImageParser(),
				new PcxImageParser(), new PngImageParser(),
				new PnmImageParser(), new PsdImageParser(),
				new RgbeImageParser(), new TiffImageParser(),
				new WbmpImageParser(), new XbmImageParser(),
				new XpmImageParser(),
	 ...
	 * ImageParser implementations are expected to return a valid IImageMetadata
	 * object or to throw an ImageReadException if unable to process the
	 * specified byte source.
	 * 
	 * @param byteSource
	 *            A valid byte source.
	 ...
	 * a null params argument, implementations should <strong>always</strong>
	...
	/**
	 * Get image metadata from the specified file. Format-specific ImageParser
	 * implementations are expected to return a valid IImageMetadata object or
	 ...
	 * ImageParser implementations are expected to return a valid ImageInfo
	 * object or to throw an ImageReadException if unable to process the
	 * specified data.
	 * 
	 * <p>
	...
	/**
	 * Get image information from the specified file Format-specific ImageParser
	 * implementations are expected to return a valid ImageInfo object or to
	 ...
	 * 
	 * @param byteSource
	 *            A valid instance of ByteSource
	...
	/**
	 * Determines the format compliance of the specified file based on rules
	...
	/**
	 * Gets all images specified by the byte source (some formats may include
	 * multiple images within a single data source).
	 * 
	 * @param byteSource
	 *            A valid instance of ByteSource.
	...
	public List<BufferedImage> getAllBufferedImages(final ByteSource byteSource)
			throws ImageReadException, IOException {
		return getAllBufferedImages(byteSource, null);
	}
	...
	public List<BufferedImage> getAllBufferedImages(
			final ByteSource byteSource, Map<String, Object> params)
			throws ImageReadException, IOException {
		if (null == params) {
            params = new HashMap<String, Object>();
        }
		final BufferedImage bi = getBufferedImage(byteSource, params);
	...
	/**
	 * Gets all images specified by the byte array (some formats may include
	 * multiple images within a single data source).
	 * 
	 * @param bytes
	 *            A valid byte array
	...
	/**
	 * Gets all images specified by indicated file (some formats may include
	 * multiple images within a single data source).
	 * 
	 * @param file
	 *            A valid reference to a file.
	...
	/**
	 * Gets a buffered image specified by the byte source (for sources that
	 * specify multiple images, choice of which image is returned is
	...
	/**
	 * Gets a buffered image specified by the byte array (for sources that
	 * specify multiple images, choice of which image is returned is
	 * implementation dependent).
	 * 
	 * @param bytes
	 ...
	 *             In the event of unsuccessful read or access operation.
	 */
	public final BufferedImage getBufferedImage(final byte[] bytes,
			final Map<String, Object> params) throws ImageReadException,
			IOException {
	...
	/**
	 * Gets a buffered image specified by the indicated file (for sources that
	 * specify multiple images, choice of which image is returned is
	 * implementation dependent).
	 * 
	 * @param file
	 ...
	 *             In the event of unsuccessful read or access operation.
	 */
	public final BufferedImage getBufferedImage(final File file,
			final Map<String, Object> params) throws ImageReadException,
			IOException {
		if (!canAcceptExtension(file)) {
			return null;
	...
	public void writeImage(final BufferedImage src, final OutputStream os,
			final Map<String, Object> params) throws ImageWriteException,
			IOException {
	 ...
	 *             In the event of unsuccessful read or access operation.
	 */
	public final Dimension getImageSize(final File file)
			throws ImageReadException, IOException {
	 ...
	 * 
	 * @param byteSource
	 *            A valid reference to a ByteSource.
	 * @param params
	 *            Optional instructions for special-handling or interpretation
	 *            of the input data.
	 * @return A valid instance of Dimension.
	 ...
	 * Metadata Platform (EXP) standard for representing information about image
	 * content. Not all image formats support EXP infomation and even for those
	 * that do, there is no guarantee that such information will be present in
	 * an image.
	 * 
	 * @param byteSource
	 *            A valid reference to a ByteSource.
	 ...
	 *            Optional instructions for special-handling or interpretation
	 *            of the input data.
	 * @return If XMP metadata is present, a valid string; if it is not present,
	 *         a null.
	 * @throws ImageReadException
	 *             In the event that the the specified content does not conform
	 *             to the format of the specific parser implementation.
	 ...
	 *             In the event of unsuccessful read or access operation.
	 */
	public abstract String getXmpXml(ByteSource byteSource,
			Map<String, Object> params) throws ImageReadException, IOException;

	/**
	 * Get an array of bytes describing the International Color Consortium (ICC)
	 * specification for the color space of the image contained in the input
	 * byte array. Not all formats support ICC profiles.
	 * 
	 * @param bytes
	 *            A valid array of bytes.
	 ...
	 * Get an array of bytes describing the International Color Consortium (ICC)
	 * specification for the color space of the image contained in the input
	 * file. Not all formats support ICC profiles.
	 * 
	 * @param file
	 *            A valid file reference.
	 * @return If available, a valid array of bytes; otherwise, a null
	 ...
	 * Get an array of bytes describing the International Color Consortium (ICC)
	 * specification for the color space of the image contained in the input
	 * byteSoruce. Not all formats support ICC profiles.
	 * 
	 * @param byteSource
	 *            A valid ByteSource.
	 * @param params
	...
	/**
	 * Write the ImageInfo and format-specific information for the image content
	 * of the specified byte array to a string.
	 * 
	 * @param bytes
	 *            A valid array of bytes.
	 ...
	 *             In the event of unsuccessful read or access operation.
	 */
	public final String dumpImageFile(final byte[] bytes)
			throws ImageReadException, IOException {
	...
	/**
	 * Write the ImageInfo and format-specific information for the image content
	 * of the specified file to a string.
	 * 
	 * @param file
	 *            A valid file reference.
	 * @return A valid string.
	 ...
	 *             In the event of unsuccessful read or access operation.
	 */
	public final String dumpImageFile(final File file)
			throws ImageReadException, IOException {
		if (!canAcceptExtension(file)) {
			return null;
		}
	...
	/**
	 * Write the ImageInfo and format-specific information for the image content
	 * of the specified byte source to a string.
	 * 
	 * @param byteSource
	 *            A valid byte source.
	 * @return A valid string.
	...
	/**
	 * Write the ImageInfo and format-specific information for the image content
	 * of the specified byte source to a PrintWriter
	 * 
	 * @param byteSource
	 *            A valid byte source.
	 ...
	 *             In the event of unsuccessful read or access operation.
	 */
	public boolean dumpImageFile(final PrintWriter pw,
	 ...
	 * of ImageParser. Some parsers can support more than one extension (i.e.
	 * .JPEG, .JPG; .TIF, .TIFF, etc.).
	 * 
	 * @return A valid string.
	 */
	...
	/**
	 * Indicates whether the ImageParser implementation can accept the specified
	 * format
	 * 
	 * @param type
	 *            An instance of ImageFormat.
	 ...
	 * 
	 * @param filename
	 *            An valid string giving a file name or file path.
	 * @return If the parser can accept the format, true; otherwise, false.
	 */
	...
	/**
	 * Get an instance of IBufferedImageFactory based on the presence of a
	 * specification for ImagingConstants.&#46;BUFFERED_IMAGE_FACTORY within the
	 * supplied params.
	 * 
	 * @param params
	 *            A valid Map object, or a null.
	 * @return A valid instance of an implementation of a IBufferedImageFactory.
	 */
	protected IBufferedImageFactory getBufferedImageFactory(
	...
	/**
	 * A utility method to search a params specification and determine whether
	 * it contains the ImagingConstants&#46;PARAM_KEY_STRICT specification.
	 * Intended for internal use by ImageParser implementations.
	 * 
	 * @param params
	 *            A valid Map object (or a null).
	 * @return If the params specify strict format compliance, true; otherwise,
	 *         false.
	 */
	public static boolean isStrict(final Map<String, Object> params) {
		...
		return ((Boolean) params.get(ImagingConstants.PARAM_KEY_STRICT))
				.booleanValue();
	}
}
