...
/**
 * Defines an interface for specifying color assignments to floating
 * point values.
 */
public interface IPaletteEntry {
    
    /**
     * Indicates that the entry covers exactly one unique value
     * (including, potentially, Float&#46;NaN).
     * @return true if the entry covers exactly one unique value 
     */
    boolean coversSingleEntry();

    /**
     * Indicates whether the indicated floating-point value is within the
     * range covered by this palette entry and can be assigned a valid
     * color by the implementation.
     * @param f a valid floating point value, or a NaN.
     * @return true if the entry can assign a color to the entry;
     * otherwise, false.
     */
     ...
     * If the input value is not within the covered range of this instance,
     * the
     ...
     * If the input value is not within the covered range of this instance,
     * the return value is undefined (though a null return  is often used).
     * @param f a valid floating point value, or a NaN.
     * @return a valid color instance or, potentially, a null if the 
     * floating point input is not within the covered range.
     */
