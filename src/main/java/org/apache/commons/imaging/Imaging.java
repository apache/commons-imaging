 ...
 * recent information on support for specific formats, refer to <a
...
public abstract class Imaging {
	...
	/**
	 * Attempts to determine if a file contains an image recorded in a supported
	 * graphics format based on its file-name extension (for example "&#46jpg",
	 * "&#46;gif", "&#46;png", etc&#46;).
	 * 
	 * @param file
	 *            A valid File object providing a reference to a file that may
	 *            contain an image.
	 * @return true if the file-name includes a supported image format file
	 ...
	 * 
	 * @param filename
	 *            A valid string representing name of file which may contain an
	 *            image.
	...
	public static ImageInfo getImageInfo(final String filename,
			final byte[] bytes, final Map<String, Object> params)
			throws ImageReadException, IOException {
	...
	public static Dimension getImageSize(final ByteSource byteSource,
			final Map<String, Object> params) throws ImageReadException,
			IOException {
		final ImageParser imageParser = getImageParser(byteSource);
	...
	private static IImageMetadata getMetadata(final ByteSource byteSource,
			final Map<String, Object> params) throws ImageReadException,
			IOException {
		final ImageParser imageParser = getImageParser(byteSource);
	 ...
	 */
	public static String dumpImageFile(final byte[] bytes)
			throws ImageReadException, IOException {
	 ...
	 *             In the event of unsuccessful read or access operation.
	 */
	public static String dumpImageFile(final File file)
			throws ImageReadException, IOException {
	 ...
	 * evaluates its format compliance. This method returns a FormatCompliance
	 * object which includes information about the data's compliance to a
	 * specific format.
	 * 
	 * @param bytes
	 *            a valid array of bytes containing image data.
	 * @return if successful, a valid FormatCompliance object.
	 * @throws ImageReadException
	 *             in the event of unreadable data.
	 * @throws IOException
	 *             in the event of an unrecoverable I/O condition.
	 */
	 ...
	 * 
	 * @param file
	 *            valid file containing image data
	 * @return if successful, a valid FormatCompliance object.
	 * @throws ImageReadException
	 *             in the event of unreadable data.
	 * @throws IOException
	...
	private static FormatCompliance getFormatCompliance(
	...
	/**
	 * Gets all images specified by the InputStream (some formats may include
	 ...
	 *             In the event of unsuccessful read or access operation.
	 */
	public static List<BufferedImage> getAllBufferedImages(
			final InputStream is, final String filename)
			throws ImageReadException, IOException {
	...
	public static List<BufferedImage> getAllBufferedImages(final byte[] bytes)
			throws ImageReadException, IOException {
		return getAllBufferedImages(new ByteSourceArray(bytes), null);
	}
	...
	public static List<BufferedImage> getAllBufferedImages(final byte[] bytes,
			final Map<String, Object> params) throws ImageReadException,
			IOException {
		return getAllBufferedImages(new ByteSourceArray(bytes), params);
	}

	/**
	 * Gets all images specified by the file (some formats may include multiple
	 * images within a single data source).
	 * 
	 * @param file
	 *            A reference to a valid data file.
	 * @return A valid (potentially empty) list of BufferedImage objects.
	 * @throws ImageReadException
	 *             In the event that the the specified content does not conform
	 *             to the format of the specific parser implementation.
	...
	public static List<BufferedImage> getAllBufferedImages(final File file)
			throws ImageReadException, IOException {
		return getAllBufferedImages(new ByteSourceFile(file), null);
	}
	...
	public static List<BufferedImage> getAllBufferedImages(final File file,
			final Map<String, Object> params) throws ImageReadException,
			IOException {
		return getAllBufferedImages(new ByteSourceFile(file), params);
	}
	...
	private static List<BufferedImage> getAllBufferedImages(
		...
		return imageParser.getAllBufferedImages(byteSource, params);
	}
	 ...
	 * Support</a> at the main project development web site. While the Apache
	...
	/**
	 * Writes the content of a BufferedImage to a file using the specified image
	 * format. Specifications for storing the file (such as data compression,
	 ...
	 * compression, color models, metadata tags, etc.) may be specified using an
	 * optional parameters map. These specifications are defined in the
	 * ImagingConstants interface or in various format-specific implementations.
	 * <p>
	 * Image writing is not supported for all graphics formats. For the most
	 * recent information on support for specific formats, refer to <a
	 * href="http://commons.apache.org/imaging/formatsupport.html">Format
	 ...
	 * specified image format. Specifications for storing the file (such as data
	 * compression, color models, metadata tags, etc.) may be specified using an
	 * optional parameters map. These specifications are defined in the
	 * ImagingConstants interface or in various format-specific implementations.
	 * <p>
